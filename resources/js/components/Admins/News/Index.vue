...
<div class="panel box-shadow-none content-header">
  <div class="panel-body">
    <div class="col-md-12">
      <h3 class="animated fadeInLeft" style="text-align:center;font-size: 3em;">List News</h3>
    </div>
  </div>
</div>
...
<div class="col-md-12 padding-5">
  <div class="panel">
    <div class="panel-body">
      ...
      <div class="col-md-12">
<button class="btn btn-primary m-1" @click.prevent="createData"><i class="fa fa-plus"></i> Create  News </button>
...
<!-- @create --->
        ...
        <template slot="header"><h4 style="text-align:center;">Add News</h4></template>
        <template slot="body" >
            <form method="POST"  action="" @submit.prevent="addItem()" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input v-model="forms.title_id" type="text" class="form-control" required>
                        <span class="label label-danger" v-for="error of errors['title_id']">
                            {{ error }}
                        </span>
                    </div>
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="image_upload">Seo Image</label>
                    ...
                    <div class="form-group">
                        ...
                        <textarea v-model="forms.description_id" class="form-control border-bottom" aria-required="true" required></textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea v-model="forms.content_id" class="form-control border-bottom" aria-required="true" required></textarea>
                        <span class="label label-danger" v-for="error of errors['content_id']">
                            {{ error }}
                        </span>
                    </div>
...
<!-- @update --->
        ...
        <template slot="header"><h4 style="text-align:center;">Update News</h4></template>
        <template slot="body" >
...
export default {    
  ...
  data () {
    ...
    forms : {id:'',title_id:'',slug_id:'',seo_img:'',tags_id:'',description_id:'',content_id:'',created_at:'',updated_at:''},
  ...
  methods: {
	fetchIt(){
		this.loading();
        axios.get('/rajawaliadmin/check-access/web-news').then((response) => {
            if(!response.data){ 
    ...
    axios.post('/rajawaliadmin/delete-news', row).then(response => {
            if(!response.data){ 
                window.location.href = window.webURL; 
            }else{ 
                if(response.data.status == 200){
    ...
    updateItem(){
        ...
        masuk.set('title_id', this.forms.title_id)
        masuk.set('description_id', this.forms.description_id)
        masuk.set('content_id', this.forms.content_id)
        masuk.set('seo_img', this.forms.seo_img)
        masuk.set('image_upload', this.image_upload)

        axios.post('/rajawaliadmin/update-news', masuk).then(response => {
            if(!response.data){ 
                window.location.href = window.webURL; 
            }else{ 
                if(response.data.status == 200){
    ...
    addItem(){
        ...
        axios.post('/rajawaliadmin/add-news', masuk).then(response => {
            if(!response.data){ 
                window.location.href = window.webURL; 
            }else{ 
                if(response.data.status == 200){
    ...
    loadItems() {
        return axios.get(`/rajawaliadmin/get-list-news?per_page=${this.serverParams.per_page}&page=${this.serverParams.page}&sort_field=${this.serverParams.sort.field}&sort_type=${this.serverParams.sort.type}&title=${this.serverParams.columnFilters.title_id}`).then((response) => {
