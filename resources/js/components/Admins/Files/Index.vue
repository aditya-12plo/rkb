...
<div class="panel box-shadow-none content-header">
  <div class="panel-body">
    <div class="col-md-12">
      <h3 class="animated fadeInLeft" style="text-align:center;font-size: 3em;">List Files</h3>
    </div>
  </div>
</div>
...
<div class="col-md-12 padding-5">
  <div class="panel">
    <div class="panel-body">
      ...
      <div class="col-md-12">
<button class="btn btn-primary m-1" @click.prevent="createData"><i class="fa fa-plus"></i> Create File </button>
...
<!-- @create --->
        ...
        <template slot="header"><h4 style="text-align:center;">Add File</h4></template>
        <template slot="body" >
            <form method="POST"  action="" @submit.prevent="addItem()" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input v-model="forms.name" type="text" class="form-control" required>
                    ...
                    <!-- form Group -->
                    <div class="form-group">
                        <label for="image_upload">File</label>
...
<!-- @update --->
        ...
        <template slot="header"><h4 style="text-align:center;">Update Files</h4></template>
        <template slot="body" >
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="name">File location</label>
                         https://rajawalikapital.co.id/public/assets/files-upload/{{this.forms.file_name}}
                    </div>
...
export default {    
  ...
  data () {
    ...
    forms : {id:'',name:'',file_name:'',created_at:'',updated_at:''},
  ...
  methods: {	
    ...
    axios.post('/rajawaliadmin/delete-files', row).then(response => {
            if(!response.data){ 
                window.location.href = window.webURL; 
            }else{ 
                if(response.data.status == 200){
    ...
    detailData(index , row){
        var masuk = {fileName:row.file_name}; 
        axios({
            url: '/rajawaliadmin/download-files',
            method: 'POST',
            data: masuk,
            responseType: 'blob', // important
            }).then((response) => {
    ...
    updateItem(){
        ...
        axios.post('/rajawaliadmin/update-files', masuk).then(response => {
            if(!response.data){ 
                window.location.href = window.webURL; 
            }else{ 
                if(response.data.status == 200){
    ...
    addItem(){
        ...
        axios.post('/rajawaliadmin/add-files', masuk).then(response => {
            if(!response.data){ 
                window.location.href = window.webURL; 
            }else{ 
                if(response.data.status == 200){
    ...
    loadItems() {
        return axios.get(`/rajawaliadmin/get-list-files?per_page=${this.serverParams.per_page}&page=${this.serverParams.page}&sort_field=${this.serverParams.sort.field}&sort_type=${this.serverParams.sort.type}&name=${this.serverParams.columnFilters.name}`).then((response) => {
