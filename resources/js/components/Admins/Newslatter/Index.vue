...
<div class="panel box-shadow-none content-header">
  <div class="panel-body">
    <div class="col-md-12">
      <h3 class="animated fadeInLeft" style="text-align:center;font-size: 3em;">List Newslatter</h3>
    </div>
  </div>
</div>
...
export default {    
  ...
  methods: {
    ...
    downloadData(){
      ...
      let masuk = new FormData();
        ...
        masuk.set('file_name', file_name)
         axios({
            url: '/rajawaliadmin/download-newslatters',
            method: 'POST',
            data: masuk,
            responseType: 'blob',
        }).then((response) => {
    ...
    loadItems() {
        return axios.get(`/rajawaliadmin/get-list-newslatters?per_page=${this.serverParams.per_page}&page=${this.serverParams.page}&sort_field=${this.serverParams.sort.field}&sort_type=${this.serverParams.sort.type}&email=${this.serverParams.columnFilters.email}&date_start=${this.serverParams.dateRange.start}&date_end=${this.serverParams.dateRange.end}`).then((response) => {
