...
<div class="panel box-shadow-none content-header">
  <div class="panel-body">
    <div class="col-md-12">
      <h3 class="animated fadeInLeft" style="text-align:center;font-size: 3em;">List Web Contact</h3>
    </div>
  </div>
</div>
...
export default {    
  ...
  methods: {
	...
	fetchIt(){
		this.loading();
        axios.get('/rajawaliadmin/check-access/web-contact').then((response) => {
            if(!response.data){ 
    ...
    clearFilter() { 
      ...
      try {
        ...
        let contact_name = newCols.find(c => {
          return c.field == 'contact_name';
        });
        contact_name.filterOptions.filterValue = "";

        let contact_email = newCols.find(c => {
          return c.field == 'contact_email';
        });
        contact_email.filterOptions.filterValue = "";
    ...
    downloadData(){
      ...
      let masuk = new FormData();
        masuk.set('contact_email', this.serverParams.columnFilters.contact_email)
        masuk.set('contact_name', this.serverParams.columnFilters.contact_name)
        ...
        masuk.set('file_name', file_name)
         axios({
            url: '/rajawaliadmin/download-contact',
            method: 'POST',
            data: masuk,
            responseType: 'blob',
        }).then((response) => {
    ...
    loadItems() {
        return axios.get(`/rajawaliadmin/get-list-contact?per_page=${this.serverParams.per_page}&page=${this.serverParams.page}&sort_field=${this.serverParams.sort.field}&sort_type=${this.serverParams.sort.type}&contact_name=${this.serverParams.columnFilters.contact_name}&contact_email=${this.serverParams.columnFilters.contact_email}&date_start=${this.serverParams.dateRange.start}&date_end=${this.serverParams.dateRange.end}`).then((response) => {
