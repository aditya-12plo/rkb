...
<div class="panel box-shadow-none content-header">
  <div class="panel-body">
    <div class="col-md-12">
      ...
      <h4 class="animated fadeInLeft" style="text-align:center;font-size: 2em;">Langkah 12</h4>
    </div>
  </div>
</div>
...
<ol type="a">
    <li>
<div class="custom-control custom-radio">
<input type="radio" class="custom-control-input" id="disputes_through" name="disputes_through" v-model="forms.disputes_through" value="Badan Arbitrase Perdagangan Berjangka Komoditi (BAKTI)" :checked="forms.disputes_through == 'Badan Arbitrase Perdagangan Berjangka Komoditi (BAKTI)'" required>
</div>   
Badan Arbitrase Perdagangan Berjangka Komoditi (BAKTI) berdasarkan
Peraturan dan Prosedur Badan Arbitrase Perdagangan Berjangka Komoditi
(BAKTI);atau</li>
    <li>
        <div class="custom-control custom-radio">
<input type="radio" class="custom-control-input" id="disputes_through" name="disputes_through" v-model="forms.disputes_through" value="Pengadilan Negeri" :checked="forms.disputes_through == 'Pengadilan Negeri'" required>
</div> 
        Pengadilan Negeri
        <div v-if="this.checkPurposeStatus(this.forms.disputes_through)">
          <input v-model="pengadilan_negeri" type="text" class="form-control" required="" aria-required="true">
          </div>
        </li>
</ol></li>
<li>
...
<div class="form-group col-lg-12">
    ...
    <button class="btn btn-info pd-x-20" type="submit">Langkah 13</button> 
</div>
...
export default {
  ...
  data () {
    return { 
      ...
      errors: [],
        ...
        position: 'up right',
        closeBtn: true,
      forms: {id : '', job_category: '' , job_position:'', country:'' , province:'' , city:'', area:'', sub_area:'', village:'', postal_code:'' , disputes_through:'', address:'',office_authorization:'', aggrement_electronic_authorization:'',status:''},
    }
  },
  watch: { 
  ...
  methods: {
    ...
    this.loading();
        ...
        axios.get('/clients-area/create-real-account/step/12/'+this.idCode).then((response) => {
            if(!response.data){ 
                ...
                if(response.data.status == 200){ 
                    ...
                    if(this.checkPurposeStatus(response.data.data.disputes_through)){
                      this.pengadilan_negeri = response.data.data.disputes_through;
                      this.forms.disputes_through = 'Pengadilan Negeri';
                    }
    ...
    submitData(){
      this.$swal({
        ...
        confirmButtonText: 'Yes!'
          ...
          if (result.value) {
            var data = {id : this.forms.id, aggrement_electronic_authorization:this.forms.aggrement_electronic_authorization ,office_authorization:this.forms.office_authorization, disputes_through:this.forms.disputes_through,pengadilan_negeri:this.pengadilan_negeri,status:this.forms.status};
            axios.post('/clients-area/post-real-account/step-12', data).then(response => {
                if(!response.data){ 
                  window.location.href = window.webURL; 
                }else{ 
                  if(response.data.status == 200){
                        ...
                        this.$router.push({name:'addRealAccount13', params: {idnya:this.diacak(response.data.data.id)}});
                    }else{
