...
<div class="panel box-shadow-none content-header">
  <div class="panel-body">
    <div class="col-md-12">
      ...
      <h4 class="animated fadeInLeft" style="text-align:center;font-size: 2em;">Langkah 16</h4>
    </div>
  </div>
</div>
      ...
      <div class="col-md-12">
        <div class="form-row">
			...
			<b>Formulir PBK. CDDS. 09</b>
                <p style="text-align:center;"> <b>PERNYATAAN BAHWA DANA YANG DIGUNAKAN SEBAGAI MARGIN
MERUPAKAN DANA MILIK NASABAH SENDIRI</b></p>
            </div>
...
<tr>
    <td colspan="3">
<p style="text-align:left;">Dengan mengisi kolom “YA” di bawah ini, Bersama ini saya menyatakan bahwa dana yang saya gunakan untuk
...
dimaksudkan untuk melakukan pencucuian uang dan/atau pendanaan terorisme.</p>
    </td>
</tr>
...
<div class="form-group col-lg-12">
    ...
    <button class="btn btn-info pd-x-20" type="submit">Langkah 17</button> 
</div>
...
export default {
  ...
  methods: {
    ...
    this.loading();
        ...
        axios.get('/clients-area/create-real-account/step/16/'+this.idCode).then((response) => {
            if(!response.data){ 
      ...
      this.$swal({
        ...
        confirmButtonText: 'Yes!'
          ...
          if (result.value) {
            ...
            axios.post('/clients-area/post-real-account/step-16', data).then(response => {
                if(!response.data){ 
                  window.location.href = window.webURL; 
                }else{ 
                  if(response.data.status == 200){
                        ...
                        this.$router.push({name:'addRealAccount17', params: {idnya:this.diacak(response.data.data.id)}});
                    }else{
